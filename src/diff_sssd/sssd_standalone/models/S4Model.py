...
class S4(nn.Module):
    ...
    def forward(self, u, **kwargs): # absorbs return_output and transformer src mask
        ...
        fft_n = 2 * L
        # cuFFT has dtype/shape constraints for half precision in some torch/cuda builds.
        # Keep AMP enabled, but run FFT in fp32 when needed and cast back afterwards.
        needs_fp32_fft = (
            u.is_cuda
            and (
                (u.dtype == torch.bfloat16)
                or (u.dtype == torch.float16 and (fft_n & (fft_n - 1)) != 0)
            )
        )

        if needs_fp32_fft:
            k_f = torch.fft.rfft(k.float(), n=fft_n) # (C H L)
            u_f = torch.fft.rfft(u.float(), n=fft_n) # (B H L)
            y_f = contract('bhl,chl->bchl', u_f, k_f) # (B C H L)
            y = torch.fft.irfft(y_f, n=fft_n)[..., :L] # (B C H L)
            y = y.to(u.dtype)
        else:
            k_f = torch.fft.rfft(k, n=fft_n) # (C H L)
            u_f = torch.fft.rfft(u, n=fft_n) # (B H L)
            y_f = contract('bhl,chl->bchl', u_f, k_f) # (B C H L)
            y = torch.fft.irfft(y_f, n=fft_n)[..., :L] # (B C H L)
