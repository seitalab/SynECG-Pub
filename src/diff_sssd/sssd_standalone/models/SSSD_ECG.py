...
class Residual_group(nn.Module):
    def __init__(self, res_channels, skip_channels, num_res_layers, 
        ...
        # The final block's residual output is not consumed by downstream computation
        # (only aggregated skip connections are used), so its res_conv parameters never
        # receive gradients. Mark them frozen to avoid DDP unused-parameter reduction errors.
        if self.num_res_layers > 0:
            for p in self.residual_blocks[-1].res_conv.parameters():
    ...
    def forward(self, input_data):
        ...
        diffusion_step_embed = calc_diffusion_step_embedding(diffusion_steps, self.diffusion_step_embed_dim_in, device=diffusion_steps.device)
