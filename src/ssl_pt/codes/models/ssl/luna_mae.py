...
import sys
sys.path.append("../baselines/codes/architectures")
from luna import LunaTransformerBlock
...
class LunaMaskedAutoencoder(nn.Module):
    def __init__(
        self,
        seqlen: int,
        chunk_size: int,
        ...
        self.projected_embeds = nn.Parameter(torch.Tensor(context_length, emb_dim))
        ...
        self.decoder_context_length = decoder_context_length
        self.decoder_projected_embeds = nn.Parameter(
            torch.Tensor(decoder_context_length, decoder_emb_dim)
        )
        nn.init.normal_(self.decoder_projected_embeds, mean=0.0, std=decoder_emb_dim ** -0.5)
        ...
        self.decoder_blocks = nn.ModuleList([
            LunaTransformerBlock(
                ...
                num_pheads=decoder_num_pheads,
                mlp_dim=int(decoder_emb_dim * mlp_ratio),
                batch_first=batch_first,
            )
    ...
    def initialize_weights(self):
        pos_embed = get_1d_sincos_pos_embed(
            self.pos_embed.shape[-1], int(self.chunk_embed.num_chunks), cls_token=True
        )
    ...
    def forward_encoder(self, x, mask_ratio=0.0):
        ...
        projected_embedded = self.projected_embeds.unsqueeze(0).expand(
            x.size(0), self.context_length, x.size(-1)
        )

        for blk in self.blocks:
            x, projected_embedded = blk(x, projected_embedded)
        x = self.norm(x)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        decoder_projected_embedded = self.decoder_projected_embeds.unsqueeze(0).expand(
            x.size(0), self.decoder_context_length, x.size(-1)
        )

        for blk in self.decoder_blocks:
            x, decoder_projected_embedded = blk(x, decoder_projected_embedded)
