...
class MegaMaskedAutoencoder(nn.Module):
        ...
        **mega_kwargs
        ...
        for _ in range(depth):
            self.encoder_layers.append(nn.ModuleList([
                ...
                norm_layer(emb_dim),
                FeedForward(dim=emb_dim, ff_mult=ff_mult),
                norm_layer(emb_dim)
            ]))
        ...
        for _ in range(decoder_depth):
            self.decoder_layers.append(nn.ModuleList([
                MegaLayer(
                    dim=decoder_emb_dim,
                    ema_heads=decoder_num_heads,
                    attn_dim_qk=decoder_qkv_dim,
                    attn_dim_value=decoder_ff_dim,
                    causal=False,
                    **mega_kwargs
                ),
                norm_layer(decoder_emb_dim),
                FeedForward(dim=decoder_emb_dim, ff_mult=ff_mult),
                norm_layer(decoder_emb_dim)
            ]))
    ...
    def forward_encoder(self, x, mask_ratio=0.0):
        ...
        for mega_layer, mega_norm, ff, ff_norm in self.encoder_layers:
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        for mega_layer, mega_norm, ff, ff_norm in self.decoder_layers:
            mega_maybe_prenorm = mega_norm if pre_norm else identity
            ff_maybe_prenorm = ff_norm if pre_norm else identity
